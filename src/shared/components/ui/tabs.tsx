...
const TabList = <T extends object>({
	...
	return (
		<TabListContext value={{ selectionIndicator, variant }}>
			<TabListPrimitive
				ref={ref}
				data-slot="tab-list"
				...
				className={composeRenderProps(className, (className, { orientation }) =>
					twMerge([
						...
						variant === "pill" && [
							"h-14 w-fit items-center gap-1 rounded-full",
							"border border-white/10 bg-fg/5 backdrop-blur-xl",
							"px-8 shadow-black/10 shadow-xl",
						],
						variant === "default" && [
							orientation === "horizontal" &&
...
const Tab = ({ className, ref, ...props }: TabProps) => {
	...
	const { selectionIndicator, variant } = useTabListContext()
	return (
		<TabPrimitive
			{...props}
			data-slot="tab"
			ref={ref}
			className={cx(
				"group/tab [--tab-gutter:var(--tab-gutter-x)]",
				variant === "pill" && [
					"relative z-10 flex cursor-pointer items-center rounded-full",
					"px-6 py-2.5 font-semibold text-sm transition-colors duration-300",
					"text-muted-fg outline-none",
					"selected:text-fg",
				...
				variant === "default" && [
					...
					"relative flex cursor-pointer items-center whitespace-nowrap font-medium text-sm/6 outline-hidden transition [-webkit-tap-highlight-color:transparent]",
		...
		>
			{composeRenderProps(props.children, (children) => (
				<>
					{children}
					{selectionIndicator && variant === "pill" && (
						<SelectionIndicator
							data-slot="selected-indicator"
							className={twMerge(
								"pointer-events-none absolute -inset-x-0 inset-y-1 rounded-full",
								"border border-white/20 bg-white/10 backdrop-blur-md",
								"shadow-black/10 shadow-lg",
								"transition-all duration-300 ease-out motion-safe:transition-[translate,width]",
							)}
						/>
					)}
					{selectionIndicator && variant === "default" && (
						<SelectionIndicator
							data-slot="selected-indicator"
							className={twMerge(
